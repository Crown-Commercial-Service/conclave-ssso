...
namespace CcsSso.Core.PPONScheduler.Service
{
  public class PPONService : IPPONService
  {
    ...
    {
      if (pPONDetails != null && pPONDetails.Id != null)
      {
        _logger.LogError("PPON Id: {0}.", pPONDetails.Id);
        await _ciiService.AddSchemeAsync(ciiOrganisationId, "GB-PPG", pPONDetails.Id, string.Empty);
      }
      else
      {
    ...
    private async Task<PPONDetails> GetPPONDetailsAsync(string ciiOrganisationId)
    {
      ...
      var response = await client.GetAsync("identifiers/" + ciiOrganisationId + "/ppon");

      if (response.IsSuccessStatusCode)
      {
        var content = await response.Content.ReadAsStringAsync();
        var result = JsonConvert.DeserializeObject<PPONDetails>(content);
        return result;
      }
      else
      {
