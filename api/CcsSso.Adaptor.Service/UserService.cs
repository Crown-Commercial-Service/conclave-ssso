...
namespace CcsSso.Adaptor.Service
{
  public class UserService : IUserService
  {
    ...
    public async Task<Dictionary<string, object>> GetUserAsync(string userName)
    {
      ...
      if (conclaveEntityMappingDictionary.Any(g => g.Key == ConclaveEntityNames.Salesforce))
      {
        var salesforceInfo = await _ciiService.GetSalesforceInfoAsync(userResponse.OrganisationId);
        var salesforceInfoDictionary = _attributeMappingService.GetMappedSalesforceInfo(salesforceInfo, conclaveEntityMappingDictionary[ConclaveEntityNames.Salesforce]);
        resultDictionaries.Add(salesforceInfoDictionary);
      }
