    ...
    public class GetPermissions
    {
      public static IEnumerable<object[]> NonExistingUserData =>
            new List<object[]>
            {
                new object[]
                {
                  "user1@mail.com","123",2, new string[] { "PERM1DIGIT", "PERM2DIGIT" }

                },
                new object[]
                {
                  "user1@mail.com","456",2, new string[]{"PERM3DIGIT","PERM4DIGIT" }
                },
            };

      [Theory]
      [MemberData(nameof(NonExistingUserData))]
      public async Task WhenUserGetData_Returns(string userName, string serviceClientId, int count, string[] expectedResults)
      {
        await DataContextHelper.ScopeAsync(async dataContext =>
        {
          var userService = await GetUserService(dataContext);
          var result = await userService.GetPermissions(userName, serviceClientId);
