    ...
    public async Task Invoke(HttpContext context)
    {
      ...
      if (path != "swagger/index.html" && path != "swagger/v1/swagger.json")
      {
        ...
        finally
        {
          ...
          Console.WriteLine($"SECURITY-API-LOGS:- Method: {method}, Path: {fullPath}, Query: {queryString}, Token: {token}, ApiKey: {apiKey}, RequestBody: {JsonConvert.SerializeObject(requestBodyString)}, Status: {context.Response.StatusCode}, ResponseBody: {responseBodyString}");
        }
      }
      else
      {
